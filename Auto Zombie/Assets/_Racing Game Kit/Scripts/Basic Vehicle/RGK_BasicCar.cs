...
public class RGK_BasicCar : MonoBehaviour, IRGKVehicle
    {
        ...
        public void ApplyDrive(float Throttle, float Brake, Boolean HandBrake)
        {
            ...
            if (Brake < 0)
            {
                ...
                if (EngineSpeed >0.2 && !IsReversing)
                {
                    ...
                    WheelColliders[0].brakeTorque = BrakeTorqueFront * Brake * -1;
                    WheelColliders[1].brakeTorque = BrakeTorqueFront * Brake * -1;
                    WheelColliders[2].brakeTorque = BrakeTorqueRear * Brake * -1;
                    WheelColliders[3].brakeTorque = BrakeTorqueRear * Brake * -1;
                }
                else
                {
                    IsReversing = true; 
                ...
                if (IsReversing)
                {
                    ...
                    if (FrontDrive)
                    {
                        WheelColliders[0].motorTorque = 50 * Brake;
                        WheelColliders[1].motorTorque = 50 * Brake;
                    }
                    if (RearDrive)
                    {
                        WheelColliders[2].motorTorque = 50 * Brake;
                        WheelColliders[3].motorTorque = 50 * Brake;
                    }
                }
            }
        ...
        void LateUpdate()
        {
            EngineRpm = (WheelColliders[0].rpm + WheelColliders[1].rpm) / 2 * GearRatio[EngineGear];
            EngineSpeed = base.rigidbody.velocity.magnitude * 3.6f;
